...
def train_dso1(
    ...
    tags = {"dso": "DSO1", "task": "binary_handover", "skip_deep": str(skip_deep)}
    ...
    cm_xgb_path = os.path.join(model_out_dir, "cm_xgb_dso1.png")
    pkl_xgb_path = os.path.join(model_out_dir, "xgb_model.pkl")
    with open(pkl_xgb_path, "wb") as f:
        pickle.dump(xgb_model, f)
    _save_cm(confusion_matrix(y_test, y_pred_xgb), "Confusion Matrix -- XGBoost (DSO1)",
             cm_xgb_path, CM_LABELS, "Blues")

    if mlflow_available:
        log_model_run(EXPERIMENT_NAME, "XGBoost", xgb_params,
                      {k: v for k, v in metrics_xgb.items() if k != "model"},
                      [cm_xgb_path, pkl_xgb_path], tags)
    ...
    cm_lgbm_path = os.path.join(model_out_dir, "cm_lgbm_dso1.png")
    pkl_lgbm_path = os.path.join(model_out_dir, "lgbm_model.pkl")
    with open(pkl_lgbm_path, "wb") as f:
        pickle.dump(lgbm_model, f)
    _save_cm(confusion_matrix(y_test, y_pred_lgbm), "Confusion Matrix -- LightGBM (DSO1)",
    ...
    if mlflow_available:
        log_model_run(EXPERIMENT_NAME, "LightGBM", lgbm_params,
                      ...
                      [cm_lgbm_path, pkl_lgbm_path], tags)
    ...
    cm_rf_path = os.path.join(model_out_dir, "cm_rf_dso1.png")
    pkl_rf_path = os.path.join(model_out_dir, "rf_model.pkl")
    with open(pkl_rf_path, "wb") as f:
        pickle.dump(rf_model, f)
    _save_cm(confusion_matrix(y_test, y_pred_rf), "Confusion Matrix -- Random Forest (DSO1)",
    ...
    if not skip_deep:
        ...
        lstm_model.fit(
            ...
            callbacks=[
                ...
                ModelCheckpoint(os.path.join(model_out_dir, "lstm_dso1_best.h5"),
        ...
        cm_lstm_path = os.path.join(model_out_dir, "cm_lstm_dso1.png")
        _save_cm(confusion_matrix(y_test, y_pred_lstm), "Confusion Matrix -- BiLSTM (DSO1)",
    ...
    if not skip_deep:
        ...
        pretrainer = TabNetPretrainer(
            ...
            optimizer_params={"lr": 2e-3}, verbose=5, seed=42,
        )
        pretrainer.fit(X_train=X_tr_tn, eval_set=[X_va_tn],
        ...
        tabnet_model = TabNetClassifier(
            ...
            n_independent=2, n_shared=2, mask_type="entmax",
            optimizer_fn=torch.optim.Adam,
            optimizer_params={"lr": 2e-3}, verbose=0, seed=42,
        )
        tabnet_model.fit(X_train=X_tr_tn[:512], y_train=y_tr_tn[:512].astype(int),
        ...
        tabnet_model.fit(X_train=X_tr_tn, y_train=y_tr_tn.astype(int),
                         eval_set=[(X_va_tn, y_val.astype(int))],
                         eval_metric=["auc"], max_epochs=30, patience=5,
                         batch_size=4096, virtual_batch_size=512, weights=1)
