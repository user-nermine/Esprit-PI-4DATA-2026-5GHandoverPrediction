...
export class MonitoringComponent implements OnInit, OnDestroy {
  ...
  alertZones: any[] = [
    {
      ...
      status: 'critical',
      alerts: [
        { gnb: 'gNB-004', priority: 1, message: 'HO failure spike', time: '2m ago' },
        { gnb: 'gNB-001', priority: 2, message: 'RSRP below threshold', time: '8m ago' }
      ]
    },
    {
      zone: 'Zone B', 
      status: 'warning',
      alerts: [
        { gnb: 'gNB-002', priority: 2, message: 'Ping-pong anomaly', time: '14m ago' },
        { gnb: 'gNB-007', priority: 2, message: 'Model drift alert', time: '1h ago' }
      ]
    },
    {
      zone: 'Zone C',
      status: 'healthy',
      alerts: [
        { gnb: 'gNB-003', priority: 3, message: 'gNB restored', time: '2h ago' }
      ]
    }
  ];
  ...
  // ───────── INIT ─────────
  ngOnInit() {
    console.log('🚀 MonitoringComponent initialized - Connecting to real backend data...');
  ...
  // ───────── MAP INIT ─────────
  initMap() {
    this.map = L.map('map', { zoomControl: true }).setView([51.5136, 7.4653], 12);

    const satelliteLayer = L.tileLayer(
      'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
      { attribution: '&copy; Esri &mdash; Source: Esri, Maxar, GeoEye', maxZoom: 19 }
    );
    ...
    // Labels overlay displayed on top of satellite
  ...
  drawZones() {
    ...
    this.zones.forEach(zone => {
      ...
      const circle = L.circle([zone.lat, zone.lng], {
        radius: 1200, color: c.fill, fillColor: c.fill, fillOpacity: 0.30, weight: 3
      }).addTo(this.map);

      const halo = L.circle([zone.lat, zone.lng], {
        radius: 1400, color: c.fill, fillColor: 'transparent',
        fillOpacity: 0, weight: 1.5, dashArray: '6, 5', opacity: 0.7
      }).addTo(this.map);
  ...
  // UE animation removed — static markers only
